...
const TEST_CASES_RAW_BASE: TestCase[] = [
  ...
  },
  {
    id: "WORK-001",
    area: "Workshops",
    title: "Workshop cards show Pre-Register on the AI video class and closed CTAs on other TBD upcoming sessions",
    priority: "P1",
    roles: ["adult", "admin", "qa"],
    assignees: ["lyriq"],
    suite: "manual",
    steps: [
      "Open Workshops",
      "Confirm every card is Upcoming with date/time TBD",
      "Confirm only the 90-Minute AI Marketing Video card shows Pre-Register / pre-registration open",
      "Confirm that card (and its registration page) show a Prerequisites link with Download PDF",
      "Confirm other cards show View Registration / registration closed",
    ],
    expected: "Only the AI video workshop is open for pre-registration; Prerequisites guide is visible on that card and registration page",
    path: "workshops",
  },
  {
    id: "CONTACT-001",
  ...
  },
  {
    ...
    steps: [
      ...
      "Open Family tab → confirm Register My Kid CTA is visible",
      "Register a kid with first name, Kids age band, kid login email + password + confirm",
      "Confirm the kid appears in the linked kids list under the parent account",
    ],
    expected: "Kid profile and login are created and linked to the logged-in parent coach",
    path: "user_portal",
  },
  {
    id: "FAMILY-004",
    ...
    steps: [
      ...
      "Enter kid name, Teens band, kid login email + password + confirm (≥8 chars)",
      "Save → confirm list shows login enabled",
      "Confirm kid and parent each receive a “can log in” confirmation email",
      "Sign out → sign in with the kid login → confirm session works",
    ],
    expected: "Required kid login is created; kid and parent get confirmation emails; login works",
    path: "user_portal",
  },
  {
    id: "FAMILY-005",
    ...
    steps: [
      ...
      "Dashboard → Blueprint → use Map Blueprint to dropdown",
      "Assign to Myself, then reassign to a linked kid",
      "Confirm Family list shows the Blueprint on that kid",
    ],
    expected: "Blueprint can be tied to parent (self) or any linked kid profile",
    path: "user_portal",
  },
  {
    id: "FAMILY-006",
    ...
    steps: [
      ...
      "Open Kids Corner (Kids mode) while logged in",
      "Confirm Your assigned Side Hustle Blueprint card shows top matches",
      "Repeat for Teens mode with a Teens Blueprint if available",
    ],
    expected: "Kid page surfaces the Blueprint the parent assigned to that age band",
    path: "kids",
  },
  {
    id: "FAMILY-007",
    ...
    steps: [
      ...
      "Confirm parent receives consent email",
      "Open consent link → fill parent details + create parent password + required kid password + confirm",
      "Approve → confirm kid profile is linked, parent can sign in, and kid + parent get login-ready emails",
    ],
    expected: "Consent path provisions parent + required kid login and emails both that the kid can sign in",
    path: "kids",
  },
  {
    id: "FAMILY-008",
    ...
    steps: [
      ...
      "Sign in as the kid",
      "Confirm parent receives a kid-login notification email",
      "Confirm the email names the kid and offers Dashboard / progress-report guidance",
    ],
    expected: "Parent gets a login alert for each successful kid/teen sign-in",
    path: "user_portal",
  },
  {
    id: "FAMILY-009",
    ...
    steps: [
      ...
      "Select Daily, then Weekly, then Off",
      "Confirm preference persists after refresh",
      "With Daily/Weekly on, confirm cron/digest path can send a parent progress report (staging/admin check OK)",
    ],
    expected: "Parent can opt into daily or weekly kid progress digests (or turn them off)",
    path: "user_portal",
  },
  {
    id: "FAMILY-010",
    ...
    steps: [
      ...
      "Have a kid Join with that parent email → open consent link",
      "Approve without setting a new parent password",
      "Confirm kid appears under that parent's Dashboard → Family list",
    ],
    expected: "Existing parents are not forced to recreate credentials; kid still links correctly",
    path: "kids",
  },
  {
    id: "FAMILY-011",
    ...
    steps: [
      ...
      "Dashboard → Family → confirm each registered kid shows a Kids/Teens dashboard link by their name",
      "Expand that kid's Blueprints details → confirm top matches show label + % match (same style as adult Blueprint tab)",
      "Tap the dashboard link → Kids/Teens Corner opens for that age band",
    ],
    expected: "Parent can open the kid dashboard from Family and review assigned Blueprint matches with percentages",
    path: "user_portal",
  },
  {
    id: "FAMILY-012",
    ...
    steps: [
      ...
      "Open Dashboard from primary nav",
      "Confirm Kids/Teens Dashboard shows My Blueprint with single numbering and % match (no 1. 1. duplicates)",
      "Confirm Explore shortcuts open Match Wizard / Piggy Bank / Guides in Corner",
    ],
    expected: "Youth members get a dedicated dashboard page parallel to the parent portal",
    path: "user_portal",
  },
  {
    id: "WIZ-UX-001",
