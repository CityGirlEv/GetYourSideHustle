...
const TEST_CASES_RAW: TestCase[] = [
  ...
  },
  {
    ...
    steps: [
      ...
      "Register a kid with first name + Kids age band (no login yet)",
      "Confirm the kid appears in the linked kids list under the parent account",
    ],
    expected: "Kid profile is created and linked to the logged-in parent coach",
    path: "user_portal",
  },
  {
    id: "FAMILY-004",
    area: "Family Coach",
    title: "Parent can create optional kid login when registering a kid",
    priority: "P1",
    roles: ["adult", "qa"],
    assignees: [],
    suite: "manual",
    steps: [
      ...
      "Enter kid name, Teens band, kid login email + password (≥8 chars)",
      "Save → confirm list shows login enabled",
      "Sign out → sign in with the kid login → confirm session works",
    ],
    expected: "Optional kid login is created and usable; still linked to parent",
    path: "user_portal",
  },
  {
    id: "FAMILY-005",
  ...
  },
  {
    ...
    steps: [
      ...
      "Open consent link → fill parent details + create parent password (+ optional kid password)",
      "Approve → confirm kid profile is linked and parent can sign in with new password",
    ],
    expected: "Consent path provisions parent credentials and links the kid without requiring prior Register My Kid",
    path: "kids",
  },
  {
    id: "FAMILY-008",
