...
export function TaskSheetContent() {
  ...
  // Otherwise we default to "done" (completion behaviour).
  const focusStatus: TaskRowStatus =
    statusFilter.length === 1 ? (statusFilter[0] as TaskRowStatus) : "done";
  const focusStatusLabel: Record<TaskRowStatus, string> = {
    done: "Done",
  ...
  // Duplicate a task — clones every field except id and inserts the copy
  // immediately after the source row so it's easy to spot. The copy keeps
  // status/dates intact; tweak via inline edits afterwards.
