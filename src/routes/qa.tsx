...
export const Route = createFileRoute("/qa")({
  head: () => ({
    meta: [
      { title: "QA Dashboard — Part B Optimizer Benchmark Tool" },
      {
        name: "description",
        content:
          "Quality assurance dashboard for The Part B Optimizer Benchmark Tool test plans, scenarios, and NDAs.",
      },
      { property: "og:title", content: "QA Dashboard — Part B Optimizer Benchmark Tool" },
      { property: "og:description", content: "Internal QA dashboard for The Part B Optimizer Benchmark Tool." },
      { property: "og:url", content: "https://themedicareoptimizer.lovable.app/qa" },
      { name: "robots", content: "noindex,nofollow" },
    ],
    links: [{ rel: "canonical", href: "https://themedicareoptimizer.lovable.app/qa" }],
  }),
...
function QADashboard() {
    ...
    (async () => {
      const [s, c, n] = await Promise.all([
        supabase
          .from("scenarios")
          .select(
            "id, scenario_code, birth_year, zip3, created_at, wants_contact, claimed_by, assigned_agent_id",
          )
  ...
  const tasksByAssignee = useMemo(() => {
    ...
    const map = new Map<
      string,
      { total: number; done: number; in_progress: number; blocked: number; not_started: number }
    >();
    for (const r of rows) {
  ...
  if (!userHasAdminRole(user)) {
    return (
      <AppShell title="QA dashboard">
        <Card className="glass p-6">This area is restricted to administrators.</Card>
      </AppShell>
    );
  }
  ...
  return (
    <AppShell
      title="QA dashboard"
      subtitle="Quality assurance: test coverage, scenarios, and contact requests"
    >
      <div className="mb-4">
        <NdaStatusCard />
      </div>
          ...
          <div className="grid gap-4 md:grid-cols-4">
            <Card className="glass p-4">
              ...
              <div className="text-xs text-muted-foreground">
                {testStats.pass} of {total} passing
              </div>
            </Card>
            <Card className="glass p-4">
              <div className="flex items-center gap-2 text-xs uppercase tracking-wider text-muted-foreground">
              ...
              <div className="font-display text-3xl mt-1 tabular-nums text-destructive">
                {testStats.fail}
              </div>
              <div className="text-xs text-muted-foreground">
                {testStats.blocked} blocked · {testStats.not_run} not run
              </div>
            </Card>
            <Card className="glass p-4">
              <div className="flex items-center gap-2 text-xs uppercase tracking-wider text-muted-foreground">
              ...
              <div className="text-xs text-muted-foreground">
                {scenarios.filter((s) => s.wants_contact).length} want contact
              </div>
            </Card>
            <Card className="glass p-4">
              <div className="flex items-center gap-2 text-xs uppercase tracking-wider text-muted-foreground">
                ...
                return (
                  <div key={area} className="flex items-center gap-3 text-sm">
                    ...
                    <div className="tabular-nums text-xs text-muted-foreground w-24 text-right">
                      {s.pass}/{s.total}{" "}
                      {s.fail > 0 && <span className="text-destructive">· {s.fail} fail</span>}
                    </div>
                  </div>
                );
              })}
          ...
          <div className="grid gap-4 lg:grid-cols-2">
                    ...
                    {tasksByAssignee.map(([who, s]) => (
                      <tr key={who} className="border-t border-border">
                        ...
                        <td className="px-3 py-2 text-right tabular-nums">
                          {s.blocked > 0 ? (
                            <span className="text-destructive">{s.blocked}</span>
                          ) : (
                            s.blocked
                          )}
                        </td>
            ...
            <Card className="glass p-4">
              <div className="flex items-center gap-2 mb-3">
                ...
                <Link to="/testing" className="ml-auto">
                  <Button size="sm" variant="outline">
                    Open test portal
                  </Button>
                </Link>
                  ...
                  <tbody>
                    {testsByAssignee.map(([who, s]) => (
                      <tr key={who} className="border-t border-border">
                        ...
                        <td className="px-3 py-2 text-right tabular-nums">
                          {s.fail > 0 ? <span className="text-destructive">{s.fail}</span> : s.fail}
                        </td>
  ...
  return (
    <tr className="border-t border-border">
      ...
      <td className="px-3 py-2">
        <Button size="sm" variant="outline" onClick={getUrl}>
