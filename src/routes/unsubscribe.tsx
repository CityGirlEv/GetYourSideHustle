      ...
      <div className="max-w-md w-full bg-card border border-border rounded-lg shadow-sm p-8 text-center">
        ...
        {status === 'done' && (
          <p className="text-foreground">
            You've been unsubscribed{email ? <> (<strong>{email}</strong>)</> : ''}.
            You will no longer receive these emails.
          </p>
        )}
        {status === 'already' && (
          <p className="text-foreground">
            {email ? <><strong>{email}</strong> is already unsubscribed.</> : 'This address is already unsubscribed.'}
          </p>
        )}
        {status === 'invalid' && (
