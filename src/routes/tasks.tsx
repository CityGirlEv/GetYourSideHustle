...
export const Route = createFileRoute("/tasks")({
  head: () => ({
    meta: [
      { title: "Task Sheet — Part B Optimizer Benchmark Tool" },
      {
        name: "description",
        content: "Spreadsheet-style task tracker for The Part B Optimizer Benchmark Tool team.",
      },
...
function TaskSheetPage() {
  ...
  return (
    <AppShell
      title="Task Sheet"
      subtitle="Spreadsheet-style task tracker for The Part B Optimizer Benchmark Tool team."
      titleClassName="bg-clip-text bg-gradient-to-r from-emerald-500 to-teal-500 [-webkit-background-clip:text] [-webkit-text-fill-color:transparent]"
    >
      {(isQa || isAdmin) && (
