...
import {
  ...
  COMPARISON_CODE_PLACEHOLDER,
  COMPARISON_ID_LABEL,
  DEIDENTIFIED_MEANING_HEADLINE,
  DEIDENTIFIED_PII_NOTE,
  PLAN_COMPARISON_CTA_PRIVATE,
  ...
  head: () => ({
    meta: [
      { title: `${SITE_BRAND_NAME} — De-identified Plan Comparison` },
      {
        name: "description",
        content:
          "Find Medicare plans for your situation without giving up your personal information. We store only de-identified plan comparisons you control.",
      },
  ...
  return (
    <AppShell title="">
      <div className="space-y-3 lg:space-y-4">
        ...
        <div className="grid gap-4 md:grid-cols-[7fr_6fr] md:items-start md:gap-6">
          <figure className="overflow-hidden rounded-2xl border border-border shadow-md">
            <HomeCoverHero />
            <div className="shrink-0 bg-primary px-3 py-2 text-primary-foreground sm:px-4 sm:py-2.5">
          ...
          <div className="glass flex min-h-[20rem] flex-col overflow-hidden rounded-2xl text-center sm:min-h-[24rem] md:h-full md:min-h-0">
            <div className="flex min-h-0 flex-1 flex-col px-4 pb-4 pt-2 sm:px-5 sm:pb-5 sm:pt-2">
              ...
              <div className="mt-4 flex w-full min-w-0 shrink-0 flex-col items-center gap-1.5 sm:mt-5 md:mt-6">
                ...
                <div className="inline-grid grid-cols-1 gap-2">
                  <Button
                    ...
                    className="compare-plans-cta h-10 w-full justify-center border-0 px-3 py-1 text-[10px] font-bold leading-tight sm:h-11 sm:px-4 sm:text-xs"
                  >
                    {PLAN_COMPARISON_CTA_PRIVATE.toUpperCase()} →
                  </Button>
                  {voiceWizardAvailable && (
                    <Button
                      ...
                      className="compare-plans-cta h-10 w-full justify-center whitespace-normal border-0 px-3 py-1 text-[10px] font-bold leading-tight sm:h-11 sm:px-4 sm:text-xs"
                    >
                      ...
                      <span className="flex flex-col items-center leading-tight text-center">
                        <span>{PLAN_COMPARISON_CTA_PRIVATE.toUpperCase()}</span>
                        <span>with VOICE WIZARD →</span>
                      </span>
                    </Button>
                  )}
                </div>
                <p className="mt-1.5 whitespace-nowrap px-1 text-center font-display text-[9px] font-semibold italic leading-tight tracking-tight text-foreground sm:mt-2 sm:text-[10px] md:text-xs">
                  {SITE_TAGLINE}
                </p>
              </div>
                ...
                <div className="grid grid-cols-1 gap-2 text-left sm:grid-cols-3 md:grid-cols-1 lg:grid-cols-3">
                  ...
                  <div className="rounded-lg border border-primary/15 bg-white/50 px-2.5 py-2 text-[11px] sm:text-xs">
                    <div className="flex items-center gap-1.5">
                      ...
                      <strong>Your {COMPARISON_ID_LABEL}</strong>
                    </div>
                    <div className="mt-0.5 text-[10px] leading-snug text-muted-foreground sm:text-[11px]">
              ...
              <div className="mx-auto w-full max-w-[21rem] shrink-0 rounded-lg border border-border/50 bg-muted/15 px-3 py-2.5 text-left text-[10px] leading-snug text-muted-foreground sm:text-[11px]">
                <p>{PLAN_COMPARISON_EDUCATIONAL_NOTE}</p>
                <p className="mt-1.5">
              ...
              <form
                ...
                className="mx-auto w-full max-w-[18rem] shrink-0 space-y-2 pb-1 text-center sm:space-y-2.5 md:max-w-[21rem]"
              >
                <div className="text-xs font-medium text-muted-foreground sm:text-sm">
                  Already have a {COMPARISON_ID_LABEL}?
                </div>
                <div className="flex gap-2">
                  <Input
                    placeholder={COMPARISON_CODE_PLACEHOLDER}
                    value={lookupCode}
