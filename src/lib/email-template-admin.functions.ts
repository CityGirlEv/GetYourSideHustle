...
} from "@/lib/email-templates/email-branding.server";
import {
...
export const listEmailTemplates = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context }) => {
    ...
    const overrideMap = new Map((overrides ?? []).map((o) => [o.template_name, o]));
...
export const getEmailTemplateCatalog = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .handler(async ({ context }) => {
    await verifyAdmin(context.userId);
    const { data: overrides } = await supabaseAdmin
    ...
    const overriddenNames = new Set((overrides ?? []).map((o) => o.template_name));
    const doc = buildEmailTemplateCatalog({ overriddenNames });
    return {
      ...doc,
      csv: catalogToCsv(doc),
      markdown: catalogToMarkdown(doc),
    };
  });
...
export const sendEmailTemplateTest = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const tpl = findTemplate(data.name)!;
    const templateData = (
      AUTH_TEMPLATE_NAMES.has(data.name)
        ? getAuthTemplateTestData(data.name, data.recipient)
        : ((registry?.previewData ??
            tpl.sampleProps ??
            getTemplateSampleProps(data.name)) as Record<string, unknown>)
