  ...
  <Html lang="en" dir="ltr">
    ...
    <Body style={main}>
      <Container style={container}>
        <EmailHeader />
        <Heading style={h1}>
          {agentName ? `Hi ${agentName},` : 'New assignment'}
        </Heading>
        <Text style={text}>
          A new scenario
          {scenarioCode ? <> (<strong>#{scenarioCode}</strong>)</> : ''} has
          been assigned to you{clientName ? <> for <strong>{clientName}</strong></> : ''}.
        </Text>
        {scenarioUrl ? (
