    ...
    <Html lang="en" dir="ltr">
      ...
      <Body style={main}>
        <Container style={container}>
          ...
          <Heading style={h1}>
            {testerName ? `Hi ${testerName},` : 'Hello,'}
          </Heading>
          <Text style={text}>Hope this email finds you well.</Text>
          <Text style={text}>
            {newCount === 1 ? (
              <>
                A new beta test has been assigned to you on <strong>{SITE_NAME}</strong>.
                When you have a moment, please log in, open the Testing Portal, and run through
                the steps below.
              </>
            ) : newCount > 1 ? (
              <>
                <strong>{newCount}</strong> new beta tests have been assigned to you on{' '}
                <strong>{SITE_NAME}</strong>. When you have a moment, please log in and work
                through them in the Testing Portal.
              </>
            ) : (
              <>
                Here is your current beta test assignment list on <strong>{SITE_NAME}</strong>.
                Log in anytime to update status and add notes in the Testing Portal.
              </>
            )}
          </Text>
...
const rowText = { fontSize: '14px', color: '#0f172a', margin: '0 0 14px', lineHeight: '1.5' }
