...
const QaRegistrationConfirmationEmail = ({
  firstName = '',
  lastName = '',
  email = '',
  qaDevices = [],
  ...
  qaManualUrl,
  testingUrl,
}: QaRegistrationConfirmationProps) => {
  const siteUrl = resolveEmailSiteUrl()
  const loginUrl = signInUrl ?? `${siteUrl}/auth?tab=sign-in`
  const manualUrl = qaManualUrl ?? `${siteUrl}/qa-manual`
  const portalUrl = testingUrl ?? `${siteUrl}/testing`
  const greetingName = firstName.trim() || 'there'
  const devicesLabel = qaDevices.length ? qaDevices.join(', ') : ''

  return (
    <Html lang="en" dir="ltr">
      <Head />
      <Preview>Your QA registration is submitted — next steps</Preview>
      <Body style={main}>
        <Container style={container}>
          <EmailHeader siteUrl={siteUrl} />
          <Heading style={h1}>Registration submitted — next steps</Heading>
          <Text style={text}>
            Thanks, <strong>{greetingName}</strong>! Your NDA is signed and your account has been
          ...
          <Text style={listItem}>
            2. Once approved, you&apos;ll receive an <strong>email at {email || 'your address'}</strong>{' '}
            letting you know your account is active.
          </Text>
          <Text style={listItem}>
            3. Return to the sign-in tab and log in with your email and password.
          </Text>
          <Text style={listItem}>
            4. After logging in, you&apos;ll land on the Testing Portal. Open the{' '}
            <Link href={manualUrl} style={link}>
              QA Manual
            </Link>{' '}
            link at the top.
          </Text>
          ...
          {devicesLabel ? (
            <Section style={card}>
              <Text style={cardLabel}>Devices you registered for testing</Text>
              <Text style={cardValue}>{devicesLabel}</Text>
            </Section>
          ) : null}
          ...
          <Text style={text}>
            After your account is enabled, you can also go directly to the{' '}
...
export const template = {
  component: QaRegistrationConfirmationEmail,
  subject: 'Your QA registration is submitted — next steps',
  displayName: 'QA registration confirmation',
  previewData: {
    firstName: 'Jane',
    lastName: 'Doe',
...
const cardValue = { fontSize: '14px', color: '#0f172a', margin: 0, lineHeight: '1.5' }
