...
export function buildMergeContext(data: Record<string, unknown>): Record<string, string> {
  ...
  if (!out.siteName) out.siteName = 'The Medicare Optimizer'
...
export function normalizeAuthBracePlaceholders(
  content: string,
  templateName: string,
): string {
...
export function applyTemplateMergeFields(
  content: string,
...
function previewLiteralsForTemplate(
  templateName: string,
): Array<{ sample: string; key: string }> {
  ...
  for (const [key, value] of Object.entries(preview)) {
    if (typeof value === 'string' && value.length >= 4) {
      if (AUTH_TEMPLATE_NAMES.has(templateName) && (key === 'recipientName' || key === 'firstName')) {
        continue
      }
...
export function stripAuthTransactionalGreetings(content: string): string {
  return content
    .replace(
      /<h1[^>]*>\s*Hi\s+(?:Jane|\{\{(?:recipientName|firstName)\}\})\s*,?\s*<\/h1>\s*/gi,
      '',
    )
    .replace(
      /<p[^>]*>\s*Hi\s+(?:Jane|\{\{(?:recipientName|firstName)\}\})\s*,?\s*<\/p>\s*/gi,
      '',
    )
...
export function resolveTemplateContent(opts: {
  ...
  const canMerge =
    hasTemplateMergeFields(html) ||
    hasTemplateMergeFields(text) ||
    hasTemplateMergeFields(subject)
  ...
  const mergedText = pickMergedOrRendered(applyTemplateMergeFields(text, templateData), renderedText)
