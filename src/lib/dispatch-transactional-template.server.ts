...
/** Enqueue `[BCC]` copies of an outgoing email to every admin notification inbox. */
export async function enqueueAdminEmailCopies(
  supabase: SupabaseClient<any, any>,
  opts: {
    templateName: string;
    idempotencyKey: string;
  ...
  try {
    const admins = getAdminBccEmails();
    for (const adminEmail of admins) {
      ...
      if (bccEnqueueError) {
        ...
        await supabase
          ...
          .update({
            status: "failed",
            error_message: `Failed to enqueue admin BCC copy: ${bccEnqueueError.message}`,
          })
          .eq("message_id", bccMessageId);
      }
    }
  } catch (bccErr) {
