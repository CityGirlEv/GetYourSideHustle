...
describe("buildCloudOps", () => {
  it("coalesces status + severity + assignee + devAssignee + sprint of the same id into ONE push", () => {
    const changes = [
      ...
      { key: "T-1:devAssignee", testId: "T-1", field: "devAssignee" as const },
  ...
  it("turns empty strings into null so cloudPushTest clears the column", () => {
    ...
    const ops = buildCloudOps(changes, new Set(["T-1:assignee"]), {
      ...blankDraft,
      assignee: { "T-1": "" },
    });
