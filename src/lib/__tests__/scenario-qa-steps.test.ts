...
describe('splitScenarioDemographics', () => {
  it('splits cost preference from other demographics', () => {
    expect(
      splitScenarioDemographics(
        "gender = male, tobacco use = NO, income band = $50k–$75k, cost preference = 'minimize monthly'",
      ),
    ).toEqual({
      basics: 'gender = male, tobacco use = NO, income band = $50k–$75k',
...
describe('buildScenarioQaAuditSteps', () => {
  ...
  it('annotates predictability with PPO vs HMO on Step 2', () => {
    const steps = buildScenarioQaAuditSteps({
      ...
      demographics:
        "gender = male, tobacco use = YES, income band = $25k–$50k, cost preference = 'predictability'",
      conditions: 'COPD',
