...
describe("validateFailDetails", () => {
  const base = { note: "broken", stepIndex: 0 };
  ...
  it("does not require screenshot evidence", () => {
    expect(validateFailDetails({ note: "broken", stepIndex: 2 })).toBeNull();
  });
});

describe("formatFailNote", () => {
  it("prefixes a structured header onto an empty note", () => {
    const out = formatFailNote("", {
      note: "modal won't close",
      stepLabel: "3",
      noScreenshot: false,
    });
