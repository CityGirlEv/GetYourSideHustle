...
const PDF_LOADING_HTML =
  ...
  "<h1 style=\"font-size:1.125rem;margin:0 0 .5rem\">Generating PDF…</h1>" +
  "<p style=\"margin:0;color:#4b5563;font-size:.875rem\">This may take a few seconds.</p>" +
  "</main>";

/**
 * Reserve a preview tab during the user click (before async PDF work).
 * Do not pass `noopener` — modern browsers return null and block a later window.open.
 */
export function preparePdfPreviewTab(): Window | null {
  if (typeof window === "undefined") return null;
  const tab = window.open("about:blank", "_blank");
  if (!tab) return null;
  try {
    tab.document.title = "Generating PDF…";
    tab.document.body.innerHTML = PDF_LOADING_HTML;
  } catch {
...
/** Navigate a tab opened via {@link preparePdfPreviewTab} to a PDF blob URL. */
export function openPdfBlobInTab(
  tab: Window | null,
  blob: Blob,
  options?: OpenPdfBlobOptions,
): void {
  ...
  throw new Error("PDF preview could not open — popup blocked.");
}
