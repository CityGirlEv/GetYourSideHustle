...
export const EVIDENCE_ALLOWED_EXT = [
  "png", "jpg", "jpeg", "webp", "gif", "heic", "heif", "pdf", "log", "txt",
] as const;
export const EVIDENCE_ACCEPT_ATTR = "image/png,image/jpeg,image/webp,image/gif,image/heic,image/heif,application/pdf,.log,.txt";
 ...
 */
async function sniffMagicBytes(file: File): Promise<"png" | "jpeg" | "gif" | "webp" | "pdf" | "heic" | "text" | "unknown"> {
  ...
  if (head.every((c) => c === 0x09 || c === 0x0a || c === 0x0d || (c >= 0x20 && c <= 0x7e))) return "text";
  return "unknown";
}
...
export async function validateEvidenceFile(file: File): Promise<void> {
  ...
  if (!EVIDENCE_ALLOWED_EXT.includes(ext as typeof EVIDENCE_ALLOWED_EXT[number])) {
    throw new Error(`.${ext || "?"} files are not allowed. Upload a screenshot (PNG/JPG/HEIC), PDF, or .log/.txt file.`);
  }
  // Reject obviously dangerous extensions even if somehow in allowlist.
  const banned = ["exe","js","mjs","ts","sh","bat","cmd","ps1","jar","apk","dmg","msi","html","htm","svg","xml","zip","rar","7z","tar","gz","docm","xlsm","pptm"];
  if (banned.includes(ext)) throw new Error("File type is not allowed for security reasons.");

  if (file.type && !EVIDENCE_ALLOWED_MIME.includes(file.type as typeof EVIDENCE_ALLOWED_MIME[number])) {
  ...
  const okMap: Record<string, string[]> = {
    ...
    heic: ["heic"], heif: ["heic"], pdf: ["pdf"], log: ["text"], txt: ["text"],
  };
