/**
 * Apply scenarios.assigned_at migration to remote Supabase.
 *
 * Tries (in order):
 *  1. Supabase Management API (needs SUPABASE_ACCESS_TOKEN)
 *  2. npx supabase db push (needs SUPABASE_DB_PASSWORD)
 *
 * Usage:
 *   node --use-system-ca scripts/apply-scenario-assigned-at-migration.mjs
 *   node --use-system-ca scripts/apply-scenario-assigned-at-migration.mjs --verify-only
 */
...
const SQL_PATH = resolve(root, "scripts", "apply-scenario-assigned-at-migration.sql");
...
async function verify(admin) {
  ...
  if (error) {
    if (/assigned_at/i.test(error.message)) {
      console.log("scenarios.assigned_at:", error.message);
      return false;
    }
    throw error;
  }

  console.log("scenarios.assigned_at column: OK");
...
async function main() {
  ...
  if (!applied) {
    ...
    console.log(`  File: scripts/apply-scenario-assigned-at-migration.sql`);
