...
async function main() {
  ...
  const templates = [
    ...ALL_TEMPLATES.map((t) => ({
      ...
      siteUrl:
        (t.sampleProps.siteUrl as string | undefined) ?? process.env.PUBLIC_SITE_URL ?? '',
    })),
  ...
  for (const tpl of templates) {
    try {
      ...
      await sendTransactionalEmail({
        ...
        subject: subjectFor(tpl.name, tpl.defaultSubject, override?.subject ?? null, tpl.sampleProps),
        html,
        text,
